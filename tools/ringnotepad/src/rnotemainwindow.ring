	...
	func CreateMenubar 
		win1 {
				menu1 = new qmenubar(this.win1) {
					...
					subHelp {
						cChmFilePath = substr(exefolder(),"\","/")+"../documents/ring.chm"
						cPdfFilePath = substr(exefolder(),"\","/")+"../documents/ring.pdf"
						if Fexists(cChmFilePath) or Fexists(cPdfFilePath)
							subHelpLF = addmenu("Language Reference")
							subHelpLF {
								cChmFilePath = substr(exefolder(),"\","/")+"../documents/ring.chm"
								if Fexists(cChmFilePath)
									oAction = new qAction(this.win1) {
										settext("CHM File")
